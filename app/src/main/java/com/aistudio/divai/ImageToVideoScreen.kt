...
fun ImageToVideoScreen(onBack: () -> Unit = {}) {
    ...
    val picker = rememberLauncherForActivityResult(ActivityResultContracts.GetContent()) { uri ->
        ...
        runCatching {
            ...
            imageFile = null
            error = it.message ?: "Could not read the selected image."
        }
    }
    ...
    Column(
        ...
        Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            listOf("Walk", "Run", "Talk", "Wave", "Dance").forEach { option -> OutlinedButton(onClick = { motion = option }) { Text(option) } }
        }
        Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            listOf("Jump", "Sit", "Stand", "Point", "Custom").forEach { option -> OutlinedButton(onClick = { motion = option }) { Text(option) } }
        }
        Text("Camera: $camera")
        Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            listOf("Static", "Zoom", "Pan", "Follow").forEach { option -> OutlinedButton(onClick = { camera = option }) { Text(option) } }
        }
        Text("Aspect ratio")
        Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
        ...
        OutlinedTextField(value = prompt, onValueChange = { prompt = it }, modifier = Modifier.fillMaxWidth(), label = { Text("Describe the animation") })
        Button(
            enabled = imageFile != null && status.isBlank(),
            onClick = {
        ...
        output?.let { file ->
            ...
            AndroidView(factory = { VideoView(context).apply { setVideoPath(file.absolutePath); setOnPreparedListener { it.isLooping = true; start() } } }, modifier = Modifier.fillMaxWidth().height(260.dp))
